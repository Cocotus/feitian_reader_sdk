 ...
 *  @param readerName is YES if the 301 has become attached to the phone or NO if the 301 has become detached from the phone.
 */
- (void) findPeripheralReader:(NSString *)readerName;

/**
 * Notify the READERS slot state change
 *
 * This delegate is called within global queue.
 *
 * @param attached YES => attached, NO => detached
 * @param bluetoothID: The Bluetooth ID
 * @param slotArray the slot name array
 */
- (void) readerInterfaceDidChange:(BOOL)attached bluetoothID:(NSString *)bluetoothID andslotnameArray:(NSArray*)slotArray;

/**
 * Notify the CARD state change
 *
 * This delegate is called within global queue.
 *
 * @param attached YES => inserted, NO => ejected
 * @param slotname the slot name which the card inserted or ejected
 */
...
/**
 *  Select the card slot by slot name
 *  @return YES if success, others failed
 */
-(BOOL)selectSlotname:(NSString*)slotName;
